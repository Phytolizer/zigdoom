...
void D_DoAdvanceDemo (void)
{
    ...
    if (gamevariant == bfgedition && !cext_strcasecmp(pagename, "TITLEPIC")
...
static void SetMissionForPackName(const char *pack_name)
{
    ...
    {
        if (!cext_strcasecmp(pack_name, packs[i].name))
        {
...
void D_IdentifyVersion(void)
{
    ...
    if (gamemission == none)
    {
        ...
        {
            if (!cext_strncasecmp(lumpinfo[i]->name, "MAP01", 8))
            {
                gamemission = doom2;
                break;
            }
            else if (!cext_strncasecmp(lumpinfo[i]->name, "E1M1", 8))
            {
                gamemission = doom;
                break;
            }
